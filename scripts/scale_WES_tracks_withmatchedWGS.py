...
def apply_pon_multivariate_gam_correction(raw_wes_depth, pon_median, offtarget_gc, mappability):
    """
...
#    lo, hi = df_fit['y'].quantile([0.01, 0.99])
 ...
 #       (df_fit['y'] >= lo) & (df_fit['y'] <= hi) &
    ...
    X_train = df_train[['gc', 'map']].values
...
def main():
    ...
    mappability    = df_pon['mappability'].values      
    dist_to_target = df_pon['dist_to_target'].values   
    ...
    for bam_path in tumor_bams:
        ...
        # Step A: Count off-target depth with mosdepth (WES)
        ...
        # Step A2: Matched tumor WGS depth evaluation
        if WGS_BAM:
        ...
        # Step B: Compute PoN-anchored ratios with multi-variable GAM correction
        print(f"    -> Computing PoN-anchored ratios and fitting GAM multi-variable correction...")
        depth_scaled = apply_pon_multivariate_gam_correction(raw_wes_depth, pon_median, offtarget_gc, mappability)


        # 4. QUALITY FILTERING & FLAGGING
        ...
        # Include matched tumor WGS track
